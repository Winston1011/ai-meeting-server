...
import (
	"ai-meeting-server/internal/domain"
	"ai-meeting-server/internal/lib/datastruct"
	"ai-meeting-server/internal/lib/utils"
	"context"
	"encoding/json"
	"errors"
	"fmt"
...
func (m *MeetingUsecase) Discuss(ctx http.Context, meetingFlowItem *domain.MeetingFlowItemRequest) error {
	...
	messages = append(messages, openai.ChatCompletionMessage{
		Role:    openai.ChatMessageRoleSystem,
		Content: fmt.Sprintf("当前环节是[%s]\n", flowItem.NodeInfo.NodeName),
		// 暂时取消成员介绍，会干扰AI的人设
	...
	if discussMessage, err := getDiscussMessage(meeting, flowItemId); err == nil && len(discussMessage) > 0 {
		messages = append(messages, openai.ChatCompletionMessage{
			Role:    openai.ChatMessageRoleSystem,
			Content: "以下是当前环节的先前的对话记录",
		})
		messages = append(messages, discussMessage...)
		messages = append(messages, openai.ChatCompletionMessage{
			Role:    openai.ChatMessageRoleSystem,
			Content: "先前的对话记录到此结束，供参考",
		})
	}
	// 人设 + 引导prompt
	messages = append(messages, openai.ChatCompletionMessage{
		Role:    openai.ChatMessageRoleSystem,
		Content: fmt.Sprintf("你是%s %s (直接按自己的特点回答即可，你不必重复需解释你是谁，也不必按前面发给你的对话记录的格式进行回复)", characters, flowItem.NodeInfo.ProloguePrompt),
	})
	gptReply, err := m.gpt.ChatCompletion(ctx, messages)
	if err != io.EOF {
		return err
...
func getContext(meeting *domain.Meeting, curFlowItem *domain.FlowItem, member *domain.Member) []openai.ChatCompletionMessage {
	...
	messages = append(messages, openai.ChatCompletionMessage{
		Role:    openai.ChatMessageRoleSystem,
		Content: fmt.Sprintf("主题和目标如下%s", meeting.TopicGoal),
	})
	// 4. 议程 meetingFlow
	agenda := "会议议程表如下:\n"
	meetingFlow := meeting.MeetingFlow
